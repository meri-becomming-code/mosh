...
class CanvasAPI:
    ...
    def validate_credentials(self):
        ...
        try:
            ...
            return False, "Check your internet connection and school website address."
    ...
    def upload_file(self, file_path, folder_path=None):
        ...
        try:
            res1 = requests.post(notify_url, headers=self.headers, data=payload)
            if res1.status_code != 200:
                return False, f"Step 1 Failed: {res1.text}"
            ...
            # Canvas wants a multipart/form-data POST
            with open(file_path, 'rb') as f_obj:
                files = {'file': f_obj}
                res2 = requests.post(upload_url, data=upload_params, files=files)
            
            # Step 3: Follow redirect if necessary
            # Canvas API docs say "Upon successful upload, the API will respond with a redirect... You must follow this redirect"
            # In practice, usually Step 2 returns the file object or a location header.
            if res2.status_code in [200, 201]:
                return True, res2.json()
            elif res2.status_code == 301 or res2.status_code == 308:
                redirect_url = res2.headers.get("Location")
                res3 = requests.get(redirect_url, headers=self.headers)
                if res3.status_code == 200:
                    return True, res3.json()
                return False, f"Step 3 Failed: {res3.text}"
            else:
                return False, f"Step 2 Failed: {res2.text}"
    ...
    def upload_imscc(self, file_path):
        ...
        try:
            ...
            # Step 2: Trigger the migration using the uploaded attachment
            payload = {
                "migration_type": "common_cartridge_importer",
                "pre_attachment_id": attachment_id
            }
            
            res_mig = requests.post(migration_url, headers=self.headers, data=payload)
            if res_mig.status_code in [200, 201]:
                return True, res_mig.json()
            else:
                return False, f"Migration Trigger Failed: {res_mig.text}"
