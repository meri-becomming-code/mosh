                ...
                if elem is not None:
                    ...
                    # Or sysClr (System - often white/black)
                        ...
                        if sys is not None:
                            ...
                            if last_clr:
                                theme_info["colors"][label] = f"#{last_clr}"
...
def convert_docx_to_html(docx_path, io_handler=None, log_func=None):
    ...
    try:
        ...
        def convert_image(image):
            ...
            original_alt = (
                image.alt_text if image.alt_text else f"Image from {filename}"
            )
            ...
            if io_handler:
                ...
                if mem_key in io_handler.memory:
                    ...
                    choice = io_handler.prompt_image(
                        f"   > Alt Text for {img_name} (or Enter to keep original): ",
                        img_path,
                        context=f"Context: {filename} (Word Document)",
                    ).strip()
                    if choice:
        ...
        if log_func:
            log_func(f"    ... Extracted {img_count} images/figures ...")
        ...
        if tables_removed > 0:
            print(
                f"    [LOG] Removed {tables_removed} empty tables from Word document."
            )
...
def convert_excel_to_html(xlsx_path):
    ...
    try:
        ...
        for sheet_name in wb.sheetnames:
            ...
            html_parts.append(
                f'<h3 class="excel-sheet-header">Sheet: {sheet_name}</h3>'
            )
...
def get_shape_text_styles(shape, theme=None):
    ...
    if has_bg or has_border:
        styles.append(
            "padding: 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #ddd;"
        )
...
def convert_ppt_to_html(ppt_path, io_handler=None, log_func=None):
    ...
    try:
        ...
        if theme["font"] != "inherit":
            style_overrides += (
                f"body {{ font-family: '{theme['font']}', sans-serif; }}\n"
            )

        # Apply Accent Colors to Slide Containers and Headings
        accent1 = theme["colors"].get(
            "accent1", "#4b3190"
        )  # Default purple if not found
        ...
        for i, slide in enumerate(prs.slides):
            slide_num = i + 1
            if log_func:
                log_func(f"    ... Processing Slide {slide_num}/{len(prs.slides)}...")
            ...
            html_parts.append(
                f'<div class="slide-num" style="position: absolute; top: 15px; right: 25px; font-size: 0.8em; color: #666; font-weight: bold;">Slide {slide_num}</div>'
            )
            ...
            for shape in sorted_shapes:
                ...
                if shape.has_text_frame:
                    ...
                    if shape.text_frame.paragraphs:
                        ...
                        if font_name and any(
                            f in font_name.lower()
                            for f in ["courier", "consolas", "mono", "lucida console"]
                        ):
                            is_code = True
                    ...
                    if is_code:
                        ...
                        html_parts.append(
                            f'<pre class="code-block" style="{style}">{safe_text}</pre>'
                        )
                        continue
                                ...
                                try:
                                    ...
                                    if run.font.color and run.font.color.rgb:
                                        inline_styles.append(
                                            f"color: #{run.font.color.rgb};"
                                        )
                                    if run.font.name:
                                        inline_styles.append(
                                            f"font-family: '{run.font.name}', sans-serif;"
                                        )
                                    if run.font.size:
                ...
                if shape.shape_type == MSO_SHAPE_TYPE.PICTURE:
                    try:
                        ...
                        width_px = (
                            int(shape.width / 9525) if hasattr(shape, "width") else 400
                        )
                        ...
                        slide_width = (
                            prs.slide_width if hasattr(prs, "slide_width") else 9144000
                        )
                        shape_left = shape.left if hasattr(shape, "left") else 0
                        shape_center_x = (
                            shape_left + (shape.width / 2)
                        ...
                        if io_handler:
                            ...
                            mem_key = interactive_fixer.normalize_image_key(
                                rel_path, image_full_path
                            )
                            ...
                            if mem_key in io_handler.memory:
                                ...
                                if not io_handler.api_key:
                                    io_handler.log(
                                        f"    [JEANIE] Skipped (No API Key found). Check Settings."
                                    )
                                else:
                                    ...
                                    try:
                                        ...
                                        io_handler.log(
                                            f"    [JEANIE] Generating suggestion for Slide {slide_num}..."
                                        )
                                        ai_suggestion, error_msg = (
                                            jeanie_ai.generate_alt_text_from_image(
                                                image_full_path,
                                                io_handler.api_key,
                                                context=slide_title,
                                            )
                                        ...
                                        if ai_suggestion:
                                            io_handler.log(
                                                f"    [JEANIE] Success: {ai_suggestion[:30]}..."
                                            )
                                        else:
                                            io_handler.log(
                                                f"    [JEANIE] Failed: {error_msg}"
                                            )

                                    except Exception as e:
                                        io_handler.log(
                                            f"    [JEANIE] Critical Error: {e}"
                                        )
            ...
            try:
                if slide.has_notes_slide:
                    ...
                    if notes_text:
                        html_parts.append(
                            '<div class="speaker-notes" style="margin-top: 30px; padding: 20px; background: #f9f9f9; border-left: 4px solid #4b3190; font-style: italic;">'
                        )
...
def convert_pdf_to_html(pdf_path, io_handler=None, force_ocr=False):
    ...
    try:
        ...
        for i, page in enumerate(doc):
            page_num = i + 1
            html_parts.append(
                f'<div class="page-container" id="page-{page_num}" style="margin-bottom: 30px; border-bottom: 1px solid #ccc; padding-bottom: 20px;">'
            )
            html_parts.append(
                f'<p class="note" style="font-size: 0.8em; color: #666; font-weight: bold;">Page {page_num}</p>'
            )
            ...
            try:
                ...
                if tables.tables:
                    for tab in tables:
                        ...
                        table_regions.append({"bbox": bbox, "table": tab})
            except Exception as e:
            ...
            for block in blocks:
                ...
                if table_region and id(table_region) not in inserted_tables:
                    ...
                    try:
                        ...
                        is_header = all(
                            isinstance(v, str) for v in first_row if v is not None
                        )
                ...
                for tr in table_regions:
                    ...
                    if (
                        tab_bbox[0] <= block_bbox[0]
                        and block_bbox[2] <= tab_bbox[2]
                        and tab_bbox[1] <= block_bbox[1]
                        and block_bbox[3] <= tab_bbox[3]
                    ):
                        is_in_table = True
                        break
                ...
                if block["type"] == 1:
                    try:
                        ...
                        image_filename = (
                            f"page{page_num}_img_{uuid.uuid4().hex[:6]}.png"
                        )
                        image_full_path = os.path.join(res_dir, image_filename)
                        ...
                        # [NEW] Image Optimization & Magic Transparency
                        optimize_image(
                            image_full_path, max_width=500, make_transparent=True
                        )
                        rel_path = f"web_resources/{safe_filename}/{image_filename}"
                        ...
                        if io_handler:
                            ...
                            if mem_key in io_handler.memory:
                                ...
                                choice = io_handler.prompt_image(
                                    f"   > Alt Text for Page {page_num} image (or Enter to skip): ",
                                    image_full_path,
                                    context=f"Context: PDF Page {page_num}",
                                ).strip()
                                if choice:
                                    if choice == "__DECORATIVE__":
                                        alt_text = ""
                        ...
                        html_parts.append(
                            f'\u003cimg src="{rel_path}" alt="{alt_text}" width="{width_attr}" class="content-image" style="{float_style}"\u003e'
                        )
                    except Exception as e:
                ...
                # Type 0 = Text
                    ...
                    while i < len(block_lines):
                        ...
                        while i < len(block_lines):
                            ...
                            # Check vertical gap
                            y_gap = abs(
                                next_line["y_pos"] - block_lines[i - 1]["y_pos"]
                            )
            ...
            try:
                ...
                found_count = len(
                    [
                        p
                        for p in html_parts
                        if "web_resources" in p and f"page{page_num}_img" in p
                    ]
                )

                if len(img_list) > found_count:
                    self.gui_handler.log(
                        f"   [PDF] Pass 2: Found {len(img_list) - found_count} additional images..."
                    )
                    for img_index, img in enumerate(img_list):
                        ...
                        if not os.path.exists(fallback_path):
                            ...
                            optimize_image(
                                fallback_path, max_width=800, make_transparent=True
                            )
                            ...
                            if io_handler:
                                choice = io_handler.prompt_image(
                                    f"   > Missing Alt Text for PDF image on Page {page_num} (from fallback): ",
                                    fallback_path,
                                    context=f"Context: PDF Page {page_num}",
                                ).strip()
                                if choice:
                                    alt_text = (
                                        choice if choice != "__DECORATIVE__" else ""
                                    )
            ...
            for tr in table_regions:
                if id(tr) not in inserted_tables:
                    try:
                        ...
                        html_parts.append(
                            df.to_html(index=False, classes="content-table").replace(
                                'class="dataframe content-table"',
                                'class="content-table"',
                            )
                        )
                    except Exception as e:
        ...
        if is_scan:
            ...
            print(
                f"    [WARNING] PDF '{filename}' appears to be a scanned image (text blocks: {total_text_blocks}, pages: {len(doc)})."
            )
            ...
            if io_handler and io_handler.config.get("api_key"):
                ...
                for page_num_ocr in range(1, len(doc) + 1):
                    ...
                    page_imgs = [
                        p for p in html_parts if f"page{page_num_ocr}_img" in p
                    ]
                    if page_imgs:
                        ...
                        if match:
                            ...
                            text, status = jeanie_ai.generate_text_from_scanned_image(
                                img_abs_path, io_handler.config["api_key"]
                            )
                            if text:
                                ocr_parts.append(
                                    f'<div class="ocr-content" style="background: #fff; padding: 20px; border: 1px solid #eee; margin: 10px 0;">{text}</div>'
                                )

                if ocr_parts:
                    html_parts.append(
                        '<div class="ocr-results" style="margin-top: 40px; border-top: 3px solid #4b3190; padding-top: 20px;">'
                    )
        ...
        print(
            f"    [LOG] PDF Conversion: Detected {h_count} headers and {img_count} images."
        )
...
def _convert_pdf_fallback(pdf_path):
    ...
    try:
        ...
        html_parts.append('<div class="pdf-content">')
        html_parts.append(
            '<p class="note">⚠️ Text-Only Extraction (Images Missing).</p>'
        )
            ...
            if file.endswith(".html"):
                ...
                try:
                    ...
                    for img in soup.find_all("img", src=True):
                        src = img["src"]
                        clean_src = (
                            urllib.parse.unquote(src).replace("\\", "/").split("?")[0]
                        )
                        if (
                            clean_src.lower().endswith(old_base.lower())
                            or src.lower() == old_base_enc.lower()
                        ):
                            img["src"] = re.sub(
                                re.escape(old_base), new_base, src, flags=re.IGNORECASE
                            )
                            img["src"] = re.sub(
                                re.escape(old_base_enc),
                                new_base.replace(" ", "%20"),
...
def batch_update_links_in_directory(directory, filename_map, log_func=None):
    ...
    if log_func:
        log_func(
            f"🚀 Turbo Link Fixer: Updated {total_links_changed} links across {total_files_updated} files in one pass."
        )
        ...
        for file in files:
            ...
            try:
                ...
                if re.search(pattern3, content, re.IGNORECASE):
                    content = re.sub(
                        pattern3, f'title="{new_base}"', content, flags=re.IGNORECASE
                    )
                    modified = True
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "r") as zip_ref:
            ...
            for i, member in enumerate(members):
                ...
                if (
                    log_func
                    and hasattr(log_func, "__self__")
                    and hasattr(log_func.__self__, "stop_requested")
                ):
                ...
                try:
                    ...
                    for old_char, new_char in replacements:
                        normalized_member = normalized_member.replace(
                            old_char, new_char
                        )
                    ...
                    if log_func:
                        log_func(
                            f"   [WARN] Could not extract: {member} ({str(file_error)[:50]})"
                        )
                    continue
...
def create_course_package(source_dir, output_path, log_func=None):
    ...
    try:
        ...
        return (
            True,
            f"Created: {output_path} ({total_files_added} files, {size_mb:.2f} MB)",
        )
    except Exception as e:
        return False, str(e)
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if log_func:
                    log_func(f"   📦 Archived original to: {ARCHIVE_FOLDER_NAME}/")
                return new_path
            except PermissionError:
                if attempt < max_retries - 1:
                    if log_func:
                        log_func(
                            f"   ⏳ File locked, retrying archive ({attempt+1}/{max_retries})..."
                        )
                    time.sleep(1.0)
                else:
                    raise
        ...
        if log_func:
            log_func(f"   ⚠️ Could not archive file (it may be open?): {e}")
...
def batch_update_manifest_resources(root_dir, path_map):
    ...
    try:
        ...
        if replacements > 0:
            ...
            return (
                True,
                f"Turbo Manifest: {replacements} entries synchronized (including web_resources).",
            )
...
def run_janitor_cleanup(source_dir, log_func=None):
    ...
    Enhanced logic:
    - Checks for both original filename and sanitized filename versions
    - Looks recursively for converted files in subdirectories
    - Also archives XLSX files that were converted to HTML
    """
    if log_func:
        log_func("🧹 Janitor: Tidying up original files...")

    extensions_to_clean = [".docx", ".doc", ".pptx", ".ppt", ".xlsx", ".xls", ".pdf"]
    cleaned_count = 0
    files_to_archive = []

    # First pass: collect all source files and find their converted versions
    for root, dirs, files in os.walk(source_dir):
        # Don't process files already in the archive or hidden folders
        ...
        for file in files:
            ...
            if ext in extensions_to_clean:
                ...
                if has_conversion:
                    ...
                    if log_func:
                        log_func(
                            f"   📄 Found converted: {os.path.basename(file_path)}"
                        )

    # Second pass: archive the files
    for file_path in files_to_archive:
        try:
            ...
            if log_func:
                log_func(f"   ⚠️ Could not archive {os.path.basename(file_path)}: {e}")

    if log_func:
        log_func(
            f"🧹 Janitor: archived {cleaned_count} source files. (Safe for upload!)"
        )
